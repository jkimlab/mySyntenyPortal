...
session_Start();
$user_id = session_Id();
shell_exec("mkdir -p ../session/$user_id/");
if(!file_exists("../session/$user_id/cur_state")){
	shell_exec("cp ../session/.reset_state ../session/$user_id/cur_state");
	shell_exec("mkdir -p ../session/$user_id/circos");
	shell_exec("cp -r ../session/circos ../session/$user_id/circos/circos1");
	shell_exec("chmod -R 777 ../session/$user_id");
}
$tar_arr = array();
...
$circos_resolution = 0;
$circos_files = scandir("../session/$user_id/circos/");
$next_circos_num = 2;
$circos_arr_index = 0;
...
?>
	...
	<input type="button" class="msp_btn" value="Add a target" style="width:110px;height:25px;font-size:13px;float:left;margin-left:30px;padding:3px 14px 5px;" onclick="tarAdd(<?php echo "'$ref_name'"?>);">
	...
	<input type="button" class="msp_submit_btn" value="Submit" style="float:right;height:30px;" onclick="Get_img(<?php echo "'$ref_name','$next_circos_num'";?>);">
	</div></div>
	<!-- Reference Line -->
	...
	<div id="ref_chr_div" style="width:1000px; height:160px; margin-top:10px;" hidden>
		...
		<?php
			$size_F = fopen("../data/sizes/$ref_name.sizes","r");
			$tmp_chr_arr = array();
	...
	if(empty($circos_tar_arr)){
		echo "tarAdd(\"$ref_name\");\n";
	} else {
		foreach ($circos_tar_arr as $i => $tar_name){
			$j = $i+1;
			echo "tarAdd(\"$ref_name\",\"$tar_name\",\"$circos_chr_arr[$j]\");\n";
		}
	}
	?>
	</script>
	...
	<input type="button" class="msp_btn" value="Download" onclick="makeCircos_img(<?php echo "'$user_id','$circos_num'";?>);">
	</div>
	<!-- Result -->
	...
	<div id="div_result" style="height:800px;margin-top:-20px;">
		<?php
			include "../session/$user_id/circos/circos$circos_num/circos.event.svg";
		?>
	</div>
	</div>
</center>
