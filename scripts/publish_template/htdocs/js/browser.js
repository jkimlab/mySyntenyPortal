$(document).ready(function(){
	$('#ref_browser').change(function(){
		...
		window.location="synbrowser.php?REF="+rn;
	});
});
...
//SVG information
	...
	function get_spc_id(){
		...
		var CurState = 'writeCurState.php',d2 = {'S1':spc1,'S2':spc2,'ASMBL':chr,'RES':resolution};
		$.post(CurState,d2,function(response2){});
		if(resolution == 0){
			window.location="synbrowser.php?REF="+spc1+"&TAR="+spc2+"&ASMBL="+chr;
		} else {
			window.location="synbrowser.php?REF="+spc1+"&TAR="+spc2+"&RES="+resolution+"&ASMBL="+chr;
		}
	...
	function linearClick(trackName,d)
	{
		...
		var ref_common_name = hash_spc_info[ref];
		var day = new Date();
		var id= day.getTime();
		window.open("http://genome.ucsc.edu/cgi-bin/hgTracks?db="+ref+"&position="+total_chr+":"+start+"-"+end,id, 'scrollbars=yes,status=no,menubar=no,resizable=yes,width=1200,height=900');
	}
	...
	function reload_function(chr, type){
		...
		if(type == "geneSearch"){
			...
			if(resolution == 0){
				window.location="synbrowser.php?REF="+ref+"&TAR="+target+"&ASMBL="+chr+"&GENE="+Sgene;
			} else {
				window.location="synbrowser.php?REF="+ref+"&TAR="+target+"&RES="+resolution+"&ASMBL="+chr+"&GENE="+Sgene;
			}
		} else if(type == "positionSearch"){
			var Sstart = $('#sy_chr_startP_text').val();
			var Send = $('#sy_chr_endP_text').val();
			if(resolution == 0){
				window.location="synbrowser.php?REF="+ref+"&TAR="+target+"&ASMBL="+chr+"&S="+Sstart+"&E="+Send;
			} else {
				window.location="synbrowser.php?REF="+ref+"&TAR="+target+"&RES="+resolution+"&ASMBL="+chr+"&S="+Sstart+"&E="+Send;
			}	
		} else {
			$.ajaxSetup({async: false});
			var CurState = 'writeCurState.php',d2 = {'S1':spc1,'S2':spc2,'ASMBL':chr,'RES':resolution};
			$.post(CurState,d2,function(response2){});
			if(resolution == 0){
				window.location="synbrowser.php?REF="+ref+"&TAR="+target+"&ASMBL="+chr;
			} else {
				window.location="synbrowser.php?REF="+ref+"&TAR="+target+"&RES="+resolution+"&ASMBL="+chr;
			}
		}
	}
	...
	function question_box_on(type, event)
	{
		if($('#qq'+type).length == 0)
		{
			...
			{
				docu = "Results will be sent to the provided email address.";
			}
	...
	function makeBrowser_img(user_id){
		var html = d3.select("svg")
			.attr("version", 1.1)
			.attr("xmlns", "http://www.w3.org/2000/svg")
			.node().parentNode.innerHTML;
		var svg_code = html;
		var save_svg = 'saveSVG.php', d = {'type':'SynBrowser','svg':svg_code};
		$.ajaxSetup({async: false});
		$.post(save_svg,d,function(){});
		$.ajaxSetup({async: true});
		svgDownload_browser(user_id);
	}
	
	function svgDownload_browser(user_id){
		var fmt = $('#b_img_fmt option:selected').val();
		var imgsrc = '../session/'+user_id+'/browser/SynBrowser.'+fmt;
			if(UrlExists(imgsrc)){
