...
function genomeTrack(layout,tracks) {
	...
	if ("undefined" == typeof layout.color)
	{
		this.color = "153,102,0\twhite";
	}
	else
	{
		this.color = this.layout.color;
	}
...
//		.domain([0, layout.genomesize])
	...
	d3.select(layout.container).select("#sy_gene_annotation_rect").call(this.zoom);
    ...
    this.tip = d3.tip()
			...
			.html(function(d) {
				console.log(d.sTerm);
				if (typeof d.sTerm === 'undefined')
				{
					return "<strong>Gene name:&nbsp</strong> <span style='color:red'>" + d.name +"</span> <br/><strong>Position:&nbsp</strong>"+d.chr+":"+d.start + "-" + d.end + "<br/> <strong>Strand:&nbsp</strong>"+d.strand+"<br/> <strong>Synteny block:&nbsp</strong>" +d.cr;
				}
				else
				{
					if (d.sTerm !== d.name)
					{
						return "<strong>Gene name:&nbsp</strong> <span style='color:red'>" + d.name +" ("+d.sTerm+")</span> <br/><strong>Position:&nbsp</strong>"+d.chr+":"+d.start + "-" + d.end + "<br/> <strong>Strand:&nbsp</strong>"+d.strand+"<br/> <strong>Synteny block:&nbsp</strong>" +d.cr;
					}
					else
					{
						return "<strong>Gene name:&nbsp</strong> <span style='color:red'>" + d.name +"</span> <br/><strong>Position:&nbsp</strong>"+d.chr+":"+d.start + "-" + d.end + "<br/> <strong>Strand:&nbsp</strong>"+d.strand+"<br/> <strong>Synteny block:&nbsp</strong>" +d.cr;
	...
	case "synTrack":
	    ...
	    this.displaySynTrack(this.tracks[i], i);
	    break;
	case "geneTrack":
		this.itemRects[i] = this.main.append("g")
		.attr("class", this.tracks[i].trackName)
	    ...
	    this.displayGeneTrack(this.tracks[i], i);
	    break;
	case "track":
	    this.itemRects[i] = this.main.append("g")
		.attr("class", this.tracks[i].trackName)
...
genomeTrack.prototype.displayStranded = function(track, i) {
    ...
    .attr("class", function(d) {
	    return track.trackName + '_' + d.bb + '_group ' + (typeof d.feature === 'undefined' ? 'gene' : d.feature); })//;
    ...
    .each(function(d) {
		...
		d3.select(this)
		.append("rect")
		.style("fill",function(d) {if(d.suffix == 'pos'){return "#000099";}else if(d.suffix == 'neg'){return "#006600";}})
		.style("fill-opacity",function(d) {if(d.suiifx == 'pos'){return ".7";}else if(d.suffix == 'neg'){return ".7";}})
		.attr("class", function(d) {
			return track.trackName + '_' + d.suffix + ' ' + ((d.width > 5) ? (track.trackName + '_' + d.suffix + '_zoomed') : '') + ' ' + ('undefined' !== typeof d.extraclass ? d.extraclass : '');})
		.attr("width", function(d) {return d.width;})
		.attr("height", function(d) {
			...
			return (d.strand == 0 ? .65 : .65) * scale_factor * y1(1);
		    })
	...
	entering_rects.each(function(d) {
		if(typeof d.feature == 'undefined' || d.feature == 'gene') {
			...
			.each(function (d) {
				var bb = this.getBBox();
				var slice_length = x1(d.end)-x1(d.start);
				d.visible = (slice_length > bb.width);
			    })
			.attr("class", function(d) {
...
genomeTrack.prototype.displayTrack = function(track, i) {
	...
	//console.log(this.color);
	var rgb = this.color.split("\t")[0];
	var text_col = this.color.split("\t")[1];
	var visStart = this.visStart,
    visEnd = this.visEnd,
    visRange = visEnd - visStart,
    x1 = this.x1,
    ...
    .each(function (d) { 
		...
		d3.select(this).append("rect")
			...
			//		entering_rects
			d3.select(this).append("text")
			    .text(function(d) {return "Synteny block: "+d.name;})
				.style("fill",text_col)
				.style("font-weight","bold")
				.style("font","12px arial")
			    .attr("dx", "2px")
...
genomeTrack.prototype.displaySynTrack = function(track, i) {
...
genomeTrack.prototype.displayGeneTrack = function(track, i) {
	//var tooltip_pos_x = d3.event.pageX - $('#body').offset().left;
	var rgb = this.color.split("\t")[0];
	var text_col = this.color.split("\t")[1];
	var visStart = this.visStart,
    visEnd = this.visEnd,
    ...
    .each(function (d) { 
		...
		d3.select(this).append("rect")
			.style("fill",function(d) {
				return (d.strand == '+') ? "rgb(0,0,153)" : "rgb(0,102,0)";
			})
			.attr("id",function(d){return "sy_"+d.id;})
			.style("fill-opacity",".7")
		    .attr("class", function(d) {return track.trackName + ' ' + ((d.width > 5) ? (track.trackName + '_zoomed') : '' ) + ' ' + ('undefined' !== typeof d.extraclass ? d.extraclass : '');})
		    .attr("width", function(d) {return d.width; })
		    .attr("height", function(d) {
	...
	rects.selectAll("text")
		.each(function(d){
			d3.select(this)
				.on("mouseover",function(d){
					tip.show(d);
					d3.select("#sy_"+d.id).style("fill","rgb(51,153,255)").style("fill-opacity",.7);
				})
				.on("mouseout",function(d){
				tip.hide(d); 
				d3.select("#sy_"+d.id).style("fill",function(d){
					return (d.strand == '+') ? "rgb(0,0,153)" : "rgb(0,102,0)";
					})
					.style("fill-opacity",.7);});
		});
...
genomeTrack.prototype.rescale = function() {
	...
	//console.log(cfg);
    var reset_s = 0;
    //console.log(this.x1.domain()[0]+","+this.x1.domain()[1]);
	...
	//console.log(this.visStart+","+this.visEnd);
    if('undefined' !== typeof this.callbackObj) 
    {
		...
		{ 
		    for(var obj in this.callbackObj) 
		    {
				...
				{
					if ((this.x1.domain()[1] - this.x1.domain()[0]) > 1000)
					{
