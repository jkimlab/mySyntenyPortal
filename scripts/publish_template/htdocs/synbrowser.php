...
session_Start();
$user_id = session_Id();
shell_exec("mkdir -p ../session/$user_id/browser");
...
?>
	...
	if(!empty($resolution_arr)){
		$input_f = fopen("../data/browser/$ref_name.$tar_name.$resolution.linear","r");
		$geneTrack_F = "../data/browser/$ref_name.$tar_name.$resolution.geneTrack";
	} else {
		$input_f = fopen("../data/browser/$ref_name.$tar_name.linear","r");
		$geneTrack_F = "../data/browser/$ref_name.$tar_name.geneTrack";
	}
...
<input type="button" class="msp_btn" id="img_fmt_button" value="Download" onclick="makeBrowser_img(<?php echo "'$user_id'";?>);">
</div>
<br><br>
...
// Reference chromosome => color legend
	...
	$input_f = fopen("../data/browser/colors.mySyntenyPortal.conf","r");
	while($line = fgets($input_f)){
		$line = trim($line);
		$arr_tmp = split(" = ",$line);
		$rgb = $arr_tmp[1];
...
// Gene information 
	if($geneTrack_flag == 1){
		...
		if(!empty($resolution_arr)){
			$input_f = fopen("../data/browser/$ref_name.$tar_name.$resolution.geneTrack","r");
		} else {
			$input_f = fopen("../data/browser/$ref_name.$tar_name.geneTrack","r");
		}
		while($line = fgets($input_f)){
			$line = trim($line);
			$arr_tmp = split("[[:space:]]",$line);
